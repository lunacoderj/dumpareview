...
export default function AdminQueue() {
  ...
  const pendingManual = queue.filter(s => s.status === 'pending' && s.apify_status === 'pending');
  const apifyProcessed = queue.filter(s => s.status === 'pending' && s.apify_status !== 'pending');
  const completed = queue.filter(s => s.status !== 'pending');

  const renderSubmissionCard = (sub: any) => {
    const isDuplicate = sub.image_hash && hashCounts[sub.image_hash] > 1;
    const isApifyProcessed = sub.status === 'pending' && sub.apify_status && sub.apify_status !== 'pending';
    ...
    return (
      <Card key={sub.id} className={`flex flex-col overflow-hidden hover:shadow-md transition-shadow relative ${selectedIds.has(sub.id) ? 'ring-2 ring-primary' : ''}`}>
        ...
        {/* Apify Agent Verdict Section */}
        {isApifyProcessed && (
          <div className={`px-3 py-2 flex flex-col gap-1 border-b ${sub.apify_status === 'approved' ? 'bg-green-50 dark:bg-green-900/10' : 'bg-red-50 dark:bg-red-900/10'}`}>
            <span className={`text-[10px] font-bold uppercase ${sub.apify_status === 'approved' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
              Apify Verdict: {sub.apify_status}
            </span>
            <span className="text-xs text-muted-foreground">{sub.apify_reason}</span>
          </div>
        )}
        ...
        <div className="space-y-8">
          {pendingManual.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold">Pending Submissions (&lt; 6 hours)</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {pendingManual.map(renderSubmissionCard)}
              </div>
            </div>
          )}

          {apifyProcessed.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold">Processed by Apify Agent</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {apifyProcessed.map(renderSubmissionCard)}
              </div>
            </div>
          )}
          ...
          {completed.length > 0 && (
            <div className="space-y-4 pt-4 border-t">
              <h2 className="text-xl font-semibold">Completed Submissions</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
